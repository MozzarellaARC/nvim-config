return {
	...
	config = function()
		...
		-- Auto-hide barbar when diffview or undo-tree is open
		vim.api.nvim_create_autocmd({ "FileType" }, {
			pattern = { "DiffviewFiles", "DiffviewFileHistory", "undotree", "diff" },
			callback = function()
				pcall(function()
		...
		-- Restore barbar when leaving diffview or undo-tree
		vim.api.nvim_create_autocmd({ "BufEnter", "TabEnter" }, {
			callback = function()
				vim.schedule(function()
					...
					if ft ~= "undotree" and not is_diffview then
						local has_diffview = false
						local has_undotree = false
						for _, win in ipairs(vim.api.nvim_tabpage_list_wins(0)) do
							...
							if ok_buf and vim.api.nvim_buf_is_valid(buf) then
								...
								if ok_ft then
									if vim.tbl_contains(diffview_fts, bufft) then
										has_diffview = true
									elseif bufft == "undotree" then
										has_undotree = true
									end
								end
							end
						end
						if not has_diffview and not has_undotree then
